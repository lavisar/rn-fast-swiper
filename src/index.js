...
export default class extends Component {
	 ...
	 * @param  {bool} animated
	 */
	scrollTo = (targetIndex, animated = true) => {
		const currentIndex = this.state.index;
		const indexDiff = targetIndex - currentIndex;

		if (indexDiff === 0) {
			// Already at the target index, no need to scroll
			return;
		}

		this.scrollBy(indexDiff, animated);
	};
	...
	renderScrollView = (pages) => {
		if (this.props.enableAndroidViewPager) {
			...
			if (
				parseFloat(require('react-native/package.json').version) >= 0.6
			) {
						...
						if (!children) {
							return (
								<View style={pageStyle} key={i}>
									{renderItem({ item: data[page], index: i })}
								</View>
							);
						} else {
							return (
